...
namespace DBapplication
{
    public partial class Hosp_StaffMembers : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                int r = objcontroller.DeleteStaffMember(Int32.Parse(textBox216.Text));
                if (r == 0)
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                int result = objcontroller.InsertWorkIn(textBox218.Text, textBox217.Text);
                if (result == 0)
        ...
        {
            if (textBox217.Text == "" || textBox218.Text == "")
            {
                MessageBox.Show("Please make sure to fill all required fields");
            }
            else if (textBox217.TextLength != 14)
            {
                MessageBox.Show("Invalid National ID, National ID must consist of 14 numbers exactly");
            }
            else
            {
                int r = objcontroller.InsertWork(textBox217.Text, textBox218.Text);
                if (r == 0)
                    MessageBox.Show("Staff member link failed");
                else
                    MessageBox.Show("Staff Member is linked Succesfully");
            }
        }
    }
}
